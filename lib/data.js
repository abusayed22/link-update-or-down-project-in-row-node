...
// geting all the checks file list
lib.list = (dir,callback) => {
    fs.readdir(`${lib.basedir + dir}/`,(err, filsNames) => {
        if(!err && filsNames && filsNames.length > 0) {
            const trimedFileNames = [];
            filsNames.forEach(fileName => {
                trimedFileNames.push(fileName.replace('.json',''));
            })
            callback(false,trimedFileNames)
        } else {
            callback('error: no found any file name')
        }
    })
}
